...
const LeaveApprovalList = () => {
    ...
    const fetchRequests = async (isRefresh = false) => {
        try {
            ...
            if (response.status === 'Success') {
                setRequests(response.pending_approvals || []);
                setTotalPending(response.total_pending_approvals || 0);
                console.log('✅ Loaded', response.pending_approvals?.length || 0, 'requests');
            } else {
                setRequests([]);
                setTotalPending(0);
            }
            ...
            setError(err.message || 'Failed to load leave approval requests');
            setRequests([]);
        } finally {
            setIsLoading(false);
    ...
    const getLeaveTypeColor = (leaveType: string) => {
        ...
        if (type.includes('casual')) return '#2196F3';
        if (type.includes('privilege') || type.includes('earned')) return '#9C27B0';
        return '#666';
    };

    // Render individual request card
    const renderRequestCard = ({ item }: { item: LeaveApprovalRequest }) => {
        const leaveColor = getLeaveTypeColor(item.leave_type);
            ...
            <View style={styles.requestCard}>
                ...
                <View style={styles.cardHeader}>
                    <View style={styles.idBadge}>
                        <Text style={styles.idText}>#{item.Leave_ID}</Text>
                    </View>
                    {item.IsHalfDay && (
                ...
                <View style={styles.infoRow}>
                    ...
                    <View style={styles.infoContent}>
                        ...
                        <View style={styles.leaveTypeBadge}>
                            <View style={[styles.leaveTypeDot, { backgroundColor: leaveColor }]} />
                            <Text style={[styles.leaveTypeText, { color: leaveColor }]}>{item.leave_type}</Text>
                        </View>
                    </View>
                </View>
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.header}>
                <View>
                    <Text style={styles.headerTitle}>Leave Approval Requests</Text>
